...
Nineteen content slides in thirty minutes is a comfortable pace with one idea per slide. If rehearsal runs long, cut from section 3 first (fold slide 3.1 into 3.2), then trim the memory bullet in 2.2 and the governance bullet in 2.6. The per-area budgets in section 2 sum to 19:00 and match `slides/02-anatomy.md`.
...
- The gloss, said once: harness in this talk means one thing, the program that runs the loop. The loop is the idea; the harness is the code. OpenAI's line for its own platform: "the reusable part is the agent loop" (August 2026). The two meanings of "harness engineering" are not named on stage; see Appendix D.
...
**Running example:** The access agent's window holds the requester's directory record, their current entitlements, policy excerpts retrieved for this request, and the request text, labeled as untrusted input. Policy documents carry a last-reviewed date, and stale policy blocks autonomous action. That is a rule in code, not in a prompt, and the deck names it as harness code: the first of three seeds that 2.4 collects.
...
**Running example:** Six tools. Look up requester, search policy, and check entitlements are read-only. Request approval is a durable wait. Notify requester is external communication. Grant entitlement is consequential and carries a sensitivity tier; the policy engine, not the model, decides whether a grant may proceed without approval. In code, not in a prompt: the second harness seed. A revoke tool exists as the compensating action.
...
*v2's 2.4 reframed as the program that runs the loop, with v2's per-action verification folded in as the first half. Verification comes first so the walk finishes the loop ring before stepping out to the harness ring.*

- One meaning of harness, held throughout: the program that runs the loop. The loop is the idea; the harness is the code. OpenAI's line for its own platform: "the reusable part is the agent loop" (August 2026). Its division of labor backs the framing without being quoted: the application owns product context, business rules, and tools; the platform provides the agent loop and sandboxed execution.
...
**Running example:** Verification before any grant confirms the requester's identity from SSO, the policy match, and the recorded approval: three deterministic checks in code before the grant tool fires. In code, not in a prompt: the third harness seed. The harness slide then collects all three. A manager approval can take two days. The agent's run must survive a deploy in the middle, resume from its log, and never re-grant on retry. If approval times out, the request lands in a human queue with the agent's reasoning attached. Step and cost budgets stop a request that is looping on policy lookups.

**Bridge:** Your coding agent running the tests before it says done is a verifier. Subagents in your coding agent follow the single-writer rule. The context-remaining indicator is the budget.

### 2.5 Evaluation

**Approximate time:** 2 minutes 15 seconds

*v2's 2.5 without per-action verification, which moved to 2.4. The hardest new skill, named by every source.*
...
"Harness engineering" means two things in 2026. Anthropic's posts use it for the system around a production agent. OpenAI's February 2026 post and Birgitta Böckeler's essay on martinfowler.com use it for configuring a coding agent with instruction files, linters, and tests. The skills overlap, which is the talk's bridge, but the targets differ. v3 does not name the ambiguity on stage; it lives in the Q&A preface in `slides/qa.md`, where it prevents the most likely confusion.

## Appendix E: What Changed from v2 and Why

Settled with the speaker on 2026-09-13 after building section 2 as slides.

| v2 | v3 | Why |
|---|---|---|
| 2.4 the harness, then 2.5 verification and evaluation | 2.4 the harness in two halves (verify one action, then the run), then 2.5 evaluation | The map walks outward ring by ring. Verification is the loop's third step and belongs inside the loop ring; the harness ring comes after the loop is complete. Verification is a harness concern in the sources (track C §5; Böckeler's sensors; Anthropic's evaluator role). |
| "Harness" carried four senses across 2.0 and 2.4: Osmani's broad sense, the ring, OpenAI's layer definition, the vocabulary warning | One meaning, glossed once in 2.0: the program that runs the loop. The loop is the idea; the harness is the code. | The audience needs one stable referent. The sources disagree among themselves, so the talk picks one. |
| The harness area opened on a definition | It opens on the two-day approval wait, with a problem-first headline | The running example is the most concrete engineering beat in the section and now carries the definition. |
| No pointer to the harness before 2.4 | A scope phrase per area (one call, what it sees, one action, that action gated, one run, many runs, production) and a refrain, "in code, not in a prompt," seeded in 2.2, 2.3, and the verification half of 2.4, collected on the harness slide | The harness arrives expected rather than as an island. |
| Vocabulary warning in 2.4 | Q&A preface only | It cost 25 seconds on stage and competed with the one-meaning gloss. |
| Evaluation drawn inside the loop on the filled map | Evaluation on the outer ring beside operations, on both the base and the filled map | 2.5 itself says evaluation is outside the loop. |
| 2.2 at 3 minutes, section at 19:30 | 2.2 at 2:30, section at 19:00 | Matches the slide file; no reconciliation needed. |
| "Verification and evaluation gets the most time" | Evaluation keeps "the hardest new skill"; "the most time" is dropped | The claim in the sources is about evals, and 2.4 is now the longer area. Evals still recur on more slides than any other area. |
