...
- Every [BUILD] in the scripts is a native PowerPoint click, simple Appear, no motion. Clicks per slide: 3 has 3, 4 has 4, 5 has 2, 6 has 1, 7 has 1, 8 has 2, 9 has 4, 10 has 1, 11 has 2, 13 has 2, 16 has 1, 17 has 7, 19 has 2. The rest have none.
- Slide 4 dims v0 to v2 with a translucent base-colored overlay that appears on the v3 click; the cards themselves are drawn at full weight.
...
- Every diagram is native shapes and stays editable in PowerPoint. Icons (key, ticket, gate, package, speech bubble, ledger, checks) are PNGs rendered from react-icons in the accent colors.
...
- Slide 10. The deploy strike is a pink "Deploy mid-wait" marker sitting above the wait node, not a dotted node, so the resume and timeout connectors never cross.
...
- Slide 8. The six-tool table sits above the three bullets, not below them, so the table reads as the main content.
