...
- Agentic AI is the priority. Every area of the discipline is taught through one enterprise agent (the access-request agent in `outline-v3.md`, four versions v0 to v3 on the autonomy spectrum). RAG and workflows appear as earlier points on that spectrum, not as separate sections.
...
- The harness, settled 2026-09-13 after building section 2 as slides: on stage "harness" means one thing, the program that runs the loop (the loop ring and the harness ring together; the loop is the idea, the harness is the code). The two-meanings warning about "harness engineering" lives only in the Q&A preface of `slides/qa.md`, never on a slide. Per-action verification is the first half of area 2.4 and precedes the harness slide, so the walk finishes the loop ring before stepping out. Evaluation (2.5) sits on the map's outer ring beside operations. Section 2 opens each slide with a scope phrase (one call, what that call sees, one action, that action gated, one run, many runs, production) and seeds a refrain, "in code, not in a prompt," on slides 7, 8, and 9 that slide 10 collects. See `outlines/outline-v3.md` Appendix E.
