...
**Slides:** 19 (1 slide) | **Approximate time:** 1 minute | **Outline:** outline-v3.md §4
...
Here is the map one last time. [POINT: map] A model in a loop, inside a harness, inside operations. Six areas. Every one of them is something you already touched from the user side of a coding agent: the instructions file, the permission prompt, the sandbox, the tests it runs before it says done, the subagent, the deprecation email.
...
- The closing line → outline-v3.md §4.
- The bridge list (instructions file, permission prompt, sandbox, tests before done, subagent, deprecation email) → synthesis.md §2 finding 10. The tests-before-done item → track-a-discipline.md §2.7 (linters, tests, and hooks as the coding-agent harness); it is slide 9's bridge. NOT IN SYNTHESIS §4.
